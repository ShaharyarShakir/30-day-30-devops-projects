...
## Local Deployment with Floci

The resources in this project are provisioned against **Floci**, a local AWS service emulator running at `http://localhost:4566`. The Terraform configuration is adapted to work with Floci’s limitations (no NAT Gateways, Flow Logs, etc.).
...
- `alb_dns_name` – The DNS name of the Application Load Balancer. Example: `dev-alb-xxxx.elb.localhost`. Open it in your browser at `http://<alb_dns_name>` to reach the frontend.
- `rds_endpoint` – The endpoint of the RDS instance for backend database connections.
...
Make sure Floci is running before applying the configuration.
